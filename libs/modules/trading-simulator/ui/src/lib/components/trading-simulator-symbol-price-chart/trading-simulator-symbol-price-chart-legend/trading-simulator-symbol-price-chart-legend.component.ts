  ...
  template: `
    <div class="flex items-center gap-x-4">
      ...
      <div matTooltip="This price is visible to all users who inspect the simulator" class="flex items-center gap-1">
        <div class="h-4 w-4 rounded-full" [style.background-color]="ColorScheme.ACCENT_1_VAR"></div>
        <div class="text-sm">Historical Price</div>
      </div>

      <!-- point -->
      @if (isOwner()) {
        <div matTooltip="This price is visible only to the owner to the simulator" class="flex items-center gap-1">
          <div class="h-4 w-4 rounded-full" [style.background-color]="ColorScheme.ACCENT_2_VAR"></div>
          <div class="text-sm">Future Price</div>
        </div>
      }
    </div>
  `,
