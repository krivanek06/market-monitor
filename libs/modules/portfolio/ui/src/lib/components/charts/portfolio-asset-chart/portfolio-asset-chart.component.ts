...
export class PortfolioAssetChartComponent extends ChartConstructor {
  @Input({ required: true }) set data(input: PortfolioGrowthAssets[]) {
    ...
    this.formatData(input).subscribe((result) => {
  ...
  private formatData(portfolioAssets: PortfolioGrowthAssets[]): Observable<Highcharts.SeriesOptionsType[]> {
    return this.symbolsControl.valueChanges.pipe(
      startWith(this.symbolsControl.value),
      tap(() => console.log('portfolioAssets', portfolioAssets)),
