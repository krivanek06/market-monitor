...
export class StockSummaryModalComponent implements OnInit {
  ...
  historicalPeriodFormControl = new FormControl<SymbolHistoricalPeriods>(
  ...
  ngOnInit(): void {
    this.stockSummary$ = this.stocksApiService.getStockSummary(
      this.data.symbol
    );
    this.stockHistoricalPrice$ =
      this.historicalPeriodFormControl.valueChanges.pipe(
        startWith(this.historicalPeriodFormControl.value),
        switchMap((period) =>
          this.stocksApiService.getSymbolHistoricalPrices(
            this.data.symbol,
            period
          )
        )
